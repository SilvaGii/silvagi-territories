...
CreateThread(function()
    ...
    ESX.TriggerServerCallback('silvagi_territories:getTerritories', function(serverData)
...
RegisterNetEvent('silvagi_territories:updateTerritories')
AddEventHandler('silvagi_territories:updateTerritories', function(serverData)
    territoryCollection = serverData
...
RegisterNetEvent('silvagi_territories:updateBlips')
AddEventHandler('silvagi_territories:updateBlips', function(id, job)
    while true do
        Wait(1000)
...
AddEventHandler('silvagi_territories:createTerritory', function()
    ...
    TriggerServerEvent('silvagi_territories:createTerritory', {
...
AddEventHandler('silvagi_territories:deleteTerritory', function()
    ...
    TriggerServerEvent('silvagi_territories:deleteTerritory', input[1])
end)

RegisterNetEvent('silvagi_territories:updateUI')
AddEventHandler('silvagi_territories:updateUI', function(action, data)
...
local function isInTerritory(isDead)
    ...
    for k, v in pairs(territoryCollection) do
        ...
        if playerTerritoryDistance < tonumber(v.radius) and v.isTaking then
            lastTerritory = v.id
            TriggerServerEvent('silvagi_territories:updateAttenders', lastTerritory, PlayerData.identifier, PlayerData.job.name, true, checkDead)
            return true, k
        end
    end
    if lastTerritory then
        TriggerServerEvent('silvagi_territories:updateAttenders', lastTerritory, PlayerData.identifier, PlayerData.job.name, false, false)
        lastTerritory = nil
    end
    return false
end
                ...
                if playerTerritoryDistance < tonumber(v.radius) then
                    ...
                    if playerTerritoryDistance < 1.5 then
                        if IsControlJustPressed(0, 38) then
                            TriggerEvent('silvagi_territories:infoMenu', { id = v.id, job = PlayerData.job.name, label = PlayerData.job.label, name = v.name, currentOwner = v.owner, terCoords = vec3(v.coords.x, v.coords.y, v.coords.z), isTaking = v.isTaking, isCooldown = v.isCooldown, type = v.type })
                        end
                        if not showUI then
                            showUI = true
...
RegisterNetEvent('silvagi_territories:infoMenu')
AddEventHandler('silvagi_territories:infoMenu', function(terData)
        ...
        options = {
            {
                ...
                event = 'silvagi_territories:captureClient',
                args = {
            ...
            },
            {
                ...
                event = 'silvagi_territories:openStash',
                args = {
                    currentTerritory = terData
                }
            }
    ...
    local terType = {
        ['dealer'] = {
            ...
            event = 'silvagi_territories:sellList',
            args = {
                data = terData
            }
        },
        ['market'] = {
            ...
            event = 'silvagi_territories:buyList',
            args = {
                data = terData
            }
        }
...
RegisterNetEvent('silvagi_territories:letCount')
AddEventHandler('silvagi_territories:letCount', function(data)
    ...
    TriggerServerEvent('silvagi_territories:marketHandler', itemObject, data.selected.type)
end)

RegisterNetEvent('silvagi_territories:sellList')
AddEventHandler('silvagi_territories:sellList', function(args)
    ...
    for k, v in pairs(shared.itemsToSell) do
        ...
        itemList[item] = {
            ...
            event = 'silvagi_territories:letCount',
            args = {
                selected = {
                    name = label,
                    key = item,
...
RegisterNetEvent('silvagi_territories:buyList')
AddEventHandler('silvagi_territories:buyList', function(terData)
...
RegisterNetEvent('silvagi_territories:captureClient')
AddEventHandler('silvagi_territories:captureClient', function(terData)
    ...
    TriggerServerEvent('silvagi_territories:captureServer', currentTerritory.id, PlayerData.job.name, currentTerritory.name, currentTerritory.owner)
end)

RegisterNetEvent('silvagi_territories:openStash')
AddEventHandler('silvagi_territories:openStash', function(terData)
    ...
    exports.ox_inventory:openInventory('stash', { id = 'devsilvagi-Ter[' .. terData.name .. '][' .. terData.id .. ']' })
end)

RegisterNetEvent('silvagi_territories:progressBars')
AddEventHandler('silvagi_territories:progressBars', function(progressType)
...
RegisterNetEvent('silvagi_territories:captureProgress')
AddEventHandler('silvagi_territories:captureProgress', function(terKey, terData)
    ...
    while true do
        ...
        if playerTerritoryDistance > terData.radius then
            TriggerEvent('silvagi_territories:progressBars', 'stop')
            ESX.ShowNotification(translateMessage('territory_cause_distance'))
            progress = 0
            TriggerServerEvent('silvagi_territories:endCapturing', lastTerritory)
            lastTerritory = nil
            return
        end
        ...
        if progress >= 60 then
            TriggerEvent('silvagi_territories:progressBars', 'stop')
            TriggerServerEvent('silvagi_territories:captureComplete', terData.id, PlayerData.job.name, PlayerData.job.label, terData.owner)
            progress = 0
            ESX.ShowNotification(string.format(translateMessage('territory_captured'), terData.name))
            return
        end

        TriggerEvent('silvagi_territories:progressBars', 'start')
...
RegisterCommand(shared.playerCommand, function(source, args, rawCommand)
        ...
        options = {
            {
                ...
                event = 'silvagi_territories:listTerritories',
                metadata = {
    ...
    if shared.rankings then
        insert(homePage.options, {
            ...
            event = 'silvagi_territories:listRankings',
            metadata = {
...
RegisterNetEvent('silvagi_territories:listTerritories')
AddEventHandler('silvagi_territories:listTerritories', function()
...
if shared.rankings then
    RegisterNetEvent('silvagi_territories:listRankings')
    AddEventHandler('silvagi_territories:listRankings', function()
        if territoryCollection == nil then return end
        ESX.TriggerServerCallback('silvagi_territories:fetchPoints', function(pointsCollection)
